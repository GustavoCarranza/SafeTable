...
document.addEventListener("DOMContentLoaded", () => {
  fntRegistrosRestaurantes();
  validarCamposTexto();
  fntAgregarRestaurantes();
});

//Funcion para mostrar los registros en la tabla
function fntRegistrosRestaurantes() {
  tableRestaurantes = $("#tableRestaurantes").DataTable({
    ...
    ajax: {
      url: Base_URL + "/AddRestaurant/getRestaurantes",
      dataSrc: "",
    },
    columns: [
      { data: "idrestaurante", className: "text-center" },
...
//Funcion para agregar restaurantes
function fntAgregarRestaurantes() {
  const btnModal = document.getElementById("btnAgregarRestaurante");
  btnModal.addEventListener("click", () => {
    $("#modalAgregar").modal("show");
    document.querySelector("#formRestaurante").reset();

    const FormRestaurante = document.querySelector("#formRestaurante");
    FormRestaurante.onsubmit = (e) => {
      e.preventDefault();
      ...
      let ajaxUrl = Base_URL + "/AddRestaurant/setRestaurantes";
      let formData = new FormData(FormRestaurante);
...
//Funcion para editar la informacion del restaurante
function btnUpdateRestaurante(element, idrestaurante) {
  rowTable = element.parentNode.parentNode.parentNode;
  var request = window.XMLHttpRequest
    ? new XMLHttpRequest()
    : new ActiveXObject("Microsoft.XMLHTTP");
  var ajaxUrl = Base_URL + "/AddRestaurant/getRestaurante/" + idrestaurante;
  ...
  request.onreadystatechange = () => {
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status) {
        document.querySelector("#idrestaurante").value =
  ...
  const formRestauranteUpdate = document.querySelector("#formRestauranteEdit");
  formRestauranteUpdate.onsubmit = (e) => {
    e.preventDefault();
    //Creamos variables y capturamos el id de los inputs
    ...
    var ajaxUrl = Base_URL + "/AddRestaurant/updateRestaurante/" + idrestaurante;
    var formData = new FormData(formRestauranteUpdate);
    request.open("POST", ajaxUrl, true);
    request.send(formData);
...
//Funcion para eliminar restaurantes
function btnDeletedRestaurante(idrestaurante){
    Swal.fire({
        title: "Eliminar restaurante",
        text: "¿Realmente quieres eliminar el restaurante?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          ...
          var ajaxUrl = Base_URL + "/AddRestaurant/deleteRestaurante/";
          var strData = "idrestaurante=" + idrestaurante;
          request.open("POST", ajaxUrl, true);
