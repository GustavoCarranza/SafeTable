...
//Almacenamos todas las funciones en el DOM para que se ejecuten
...
//Funcion para que se muestren los registros en la tabla
function fntRegistrosReservas() {
  tableDestinations = $("#tableDestinations").DataTable({
    ...
    ajax: {
      url: Base_URL + "/Destinations/getReservas",
      dataSrc: "",
    },
    columns: [
      { data: "idreservasD", className: "text-center" },
...
//Funcion para extraer los destinations de la base
function fntDestinations() {
  ...
  var ajaxUrl = Base_URL + "/Destinations/getSelectDestinations";
  ...
  request.onreadystatechange = function () {
    if (request.readyState == 4 && request.status == 200) {
      document.querySelector("#listDestinations").innerHTML =
        request.responseText;
      document.querySelector("#listDestinations").value = 1;
      document.querySelector("#listDestinationsEdit").innerHTML =
        request.responseText;
      document.querySelector("#listDestinationsEdit").value = 1;
    }
  };
}
...
function fntAgregarReservas() {
  let modalDestinations = document.getElementById("btnModalDestinations");
  modalDestinations.addEventListener("click", () => {
    ...
    FormReserva.onsubmit = (e) => {
      ...
      const listDestinations = document.querySelector("#listDestinations");
      ...
      let ajaxUrl = Base_URL + "/Destinations/setDestinations";
...
//Funcion para visualizar la informacion de la reservacion
function btnViewReserva(idreservasD) {
  let request = window.XMLHttpRequest
    ? new XMLHttpRequest()
    : new ActiveXObject("Microsoft.XMLHTTP");
  let ajaxUrl = Base_URL + "/Destinations/getReserva/" + idreservasD;
  request.open("GET", ajaxUrl, true);
  request.send();
  request.onreadystatechange = () => {
    if (request.readyState == 4 && request.status == 200) {
      try {
        ...
        if (objData.status) {
          ...
          document.querySelector("#cellDestination").innerHTML =
            objData.data.nombre_destination;
...
//Funcion para editar la informacion de la reservacion
function btnUpdateReserva(element, idreservasD) {
  ...
  request.onreadystatechange = () => {
    ...
    if (request.readyState == 4 && request.status == 200) {
      ...
      if (objData.status) {
        document.querySelector("#idreserva").value = objData.data.idreservasD;
        document.querySelector("#listDestinationsEdit").value =
  ...
  FormReservaEdit.onsubmit = (e) => {
    ...
    const listDestinationEdit = document.querySelector(
      "#listDestinationsEdit"
    ).value;
    ...
    let ajaxURL = Base_URL + "/Destinations/updateRerserva/" + idreservasD;
...
function btnDeletedReserva(idreservasD) {
    Swal.fire({
      ...
      var ajaxUrl = Base_URL + "/Destinations/deleteReserva/";
      var strData = "idreservasD=" + idreservasD;
      request.open("POST", ajaxUrl, true);
