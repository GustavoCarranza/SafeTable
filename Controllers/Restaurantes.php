...
class Restaurantes extends Controllers
{
    ...
    public function Restaurantes()
    {
        ...
        $data['page_tag'] = "Reservas - SafeTable";
        $data['page_title'] = "Reservaciones - SafeTable";
        $data['page_name'] = "restaurantes";
        $data['page_functions_js'] = "funciones_restaurantes.js";
        $this->views->getView($this, "restaurantes", $data);
    }

    //Metodo para extraer los registros de las reservas de la BD 
    public function getReservas()
    {
        if ($_SESSION['permisosModulo']['r']) {
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                ...
                if ($_SESSION['permisosModulo']['r']) {
                    $btnView = '<button class="btn btn-secondary btn-sm" onclick="btnViewReserva(' . $arrData[$i]['idreservasR'] . ')" title = "Ver Reservacion"><i class="fas fa-eye"></i></button>';
                }
                //Si tiene permisos para editar mostramos el boton de editar
                if ($_SESSION['permisosModulo']['u']) {
                    $btnEdit = '<button class="btn btn-success btn-sm" onclick="btnUpdateReserva(this,' . $arrData[$i]['idreservasR'] . ')" title = "Actualizar Reservacion"><i class="fas fa-edit"></i></button>';
                }
                //Si tiene permisos para eliminar mostrar el boton 
                if ($_SESSION['permisosModulo']['d']) {
                    $btnDelete = '<button class="btn btn-danger btn-sm" onclick="btnDeletedReserva(' . $arrData[$i]['idreservasR'] . ')" title = "Eliminar Reservacion"><i class="fas fa-trash"></i></button>';
                }
    ...
    //Metodo para extraer los restaurantes de la BD 
    public function getSelectRestaurantes()
    {
        ...
        if (count($arrData) > 0) {
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                if ($arrData[$i]['status'] == 1) {
                    ...
                    $htmlOptions .= '<option value="' . $arrData[$i]['idrestaurante'] . '">' . $arrData[$i]['nombre'] . '</option>';
                }
            }
        }
    ...
    public function setRestaurantes()
    {
        if ($_SESSION['permisosModulo']['w']) {
            ...
            if ($_POST) {
                ...
                $listRestaurante = intval($_POST['listRestaurantes']);
                ...
                $request_rest = $this->model->insertReservas($idusuario, $listRestaurante, $strHuesped, $strApellidos, $intVilla, $strHotel, $intAdultos, $intKids, $strFecha, $strHorario, $strComentarios);
    ...
    public function getReserva($idreservasR)
    {
        if ($_SESSION['permisosModulo']['r']) {
    ...
    //Metodo para editar las reservaciones
    public function updateRerserva($idReserva)
    {
        if ($_SESSION['permisosModulo']['u']) {
            ...
            if ($_POST && $idreserva > 0) {
                    ...
                    $idusuario = $_SESSION['UserData']['usuario'];
                    $listRestaurante = intval($_POST['listRestaurantesEdit']);
                    ...
                    $request_rest = $this->model->updateRerserva($idreserva,$idusuario, $listRestaurante, $strHuesped, $strApellidos, $intVilla, $strHotel, $intAdultos, $intKids, $strFecha, $strHorario, $strComentarios);
    ...
    public function deleteReserva()
    {
         ...
         if ($_POST) {
            if ($_SESSION['permisosModulo']['d']) {
                ...
                $intId = intval($_POST['idreservasR']);
                $idusuario = $_SESSION['UserData']['usuario'];
